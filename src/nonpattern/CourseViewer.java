...
public class CourseViewer extends JFrame implements ActionListener,
	...
	public void addCourse(CourseRecord courseRecord) {
		...
		if (!alreadyExists) { // add the slider
			...
			this.setSize(LayoutConstants.xOffset + 50 + this.sliders.size()
					* (LayoutConstants.barWidth + LayoutConstants.barSpacing)+1,
					(sliders.size() + 1) * 100 + this.button.getHeight()+100);
	...
	public double getTotal(Vector<JSlider> data){
		double total = 0.0;
		for (int i = 0; i < data.size(); i++) {
			JSlider record = data.elementAt(i);
	...
	public void paint(Graphics g) {
		...
		double total = getTotal(this.sliders);
		int radius = 100;
		...
		for (int i = 0; i < sliders.size(); i++) {
			...
			g.fillArc(LayoutConstants.xOffset, LayoutConstants.yOffset + 280, 2 * radius, 2 * radius, (int) startAngle, (int) ratio);
			startAngle += ratio;
		}
